...
SW = r'''const CACHE="planet-v4";self.addEventListener("install",e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(["/","/manifest.webmanifest"]))));self.addEventListener("fetch",e=>{if(e.request.url.includes("/api/"))return;e.respondWith(caches.match(e.request).then(r=>r||fetch(e.request)))})'''
...
<section class="section card"><h2>🗺️ Mapa świata</h2><div class="map">Warstwa mapowa: następny etap. API już zwraca geometrię zdarzeń EONET i USGS.</div></section>
