...
from planet_db import db_status, init_db
from history_service import history, period_comparison
from weekly_service import generate_weekly_report, save_weekly_report, latest_weekly_report

app = FastAPI(title="Dziennik Planety", version="4.1.0")
...
async def all_data():
    ...
    try:
        ...
        await asyncio.to_thread(collect, data)
    except Exception:
        pass
    return {"updated_at": datetime.now(timezone.utc).isoformat(), "data": data}

@app.get("/api/health")
async def health():
    return {"status": "ok", "app": "Dziennik Planety", "version": "4.1.0", "utc": datetime.now(timezone.utc).isoformat()}

@app.get("/api/db-status")
async def database_status():
    return await asyncio.to_thread(db_status)

@app.get("/api/history")
async def api_history(metric: str = Query(..., min_length=1), limit: int = Query(120, ge=1, le=1000)):
    try:
        rows = await asyncio.to_thread(history, metric, limit)
        return {"metric": metric, "count": len(rows), "rows": rows}
    except Exception as exc:
        return {"metric": metric, "count": 0, "rows": [], "error": str(exc)[:300]}

@app.get("/api/compare")
async def api_compare(metric: str = Query(..., min_length=1), days: int = Query(7, ge=1, le=3650)):
    try:
        return await asyncio.to_thread(period_comparison, metric, days)
    except Exception as exc:
        return {"metric": metric, "period_days": days, "error": str(exc)[:300]}

@app.get("/api/weekly")
async def api_weekly():
    try:
        report = await asyncio.to_thread(generate_weekly_report)
        await asyncio.to_thread(save_weekly_report, report)
        return report
    except Exception as exc:
...
async function loadData(){const st=document.getElementById('status');st.textContent='Pobieranie danych…';try{const r=await fetch('/api/all');if(!r.ok)throw Error(r.status);const d=await r.json();render(d.data);st.textContent='Ostatnia aktualizacja: '+new Date(d.updated_at).toLocaleString('pl-PL');loadHistory();loadWeekly()}catch(e){st.textContent='Błąd API: '+e}}
...
document.getElementById('ocean').innerHTML=box('NOAA AMO',`ostatnie lata: ${v(o.rows?.length)} rekordów`)+box('Interpretacja','AMO nie jest globalną temperaturą oceanu. Do monitoringu SST używamy Copernicus Climate Pulse.')+`<div class="small">Copernicus Climate Pulse dostarcza dzienne szeregi ERA5.</div>`;
...
document.getElementById('events').innerHTML=box('Pożary',v(ev.counts?.wildfires))+box('Wulkany',v(ev.counts?.volcanoes))+box('Burze',v(ev.counts?.severeStorms));document.getElementById('space').innerHTML=box('Kp',v(sp.latest_kp?.kp_index??sp.latest_kp?.kp))+box('Cykl słoneczny',v(sp.latest_solar_cycle?.solar_cycle_number??sp.latest_solar_cycle?.cycle));document.getElementById('bio').innerHTML=box('GBIF Polska — wszystkie rekordy',v(b.poland_occurrences_total))+box('GBIF Polska — bieżący rok',v(b.poland_occurrences_current_year))+`<div class="small">To wskaźnik aktywności obserwacyjnej, nie bezpośredni indeks liczebności gatunków.</div>`;
...
function drawHistory(rows){const c=document.getElementById('historyChart'),ctx=c.getContext('2d'),w=c.clientWidth,h=c.clientHeight,d=devicePixelRatio||1;c.width=w*d;c.height=h*d;ctx.scale(d,d);ctx.clearRect(0,0,w,h);const a=rows.slice().reverse(),ys=a.map(z=>Number(z.value)).filter(Number.isFinite);if(ys.length<2)return;const mn=Math.min(...ys),mx=Math.max(...ys);ctx.beginPath();a.forEach((p,j)=>{const y=Number(p.value);if(!Number.isFinite(y))return;const xx=10+j*(w-20)/Math.max(1,a.length-1),yy=h-10-(y-mn)/(mx-mn||1)*(h-20);j?ctx.lineTo(xx,yy):ctx.moveTo(xx,yy)});ctx.strokeStyle='#9be3c4';ctx.lineWidth=2;ctx.stroke()}
async function loadHistory(){try{const [h,c]=await Promise.all([fetch('/api/history?metric=global_temperature_daily&limit=90').then(r=>r.json()),fetch('/api/compare?metric=global_temperature_daily&days=7').then(r=>r.json())]);drawHistory(h.rows||[]);document.getElementById('comparison').innerHTML=box('Ostatnie 7 dni',v(c.current?.average))+box('Poprzednie 7 dni',v(c.previous?.average))+box('Różnica',v(c.delta))+box('Zmiana %',c.delta_percent==null?'—':v(c.delta_percent.toFixed(2))+'%') }catch(e){document.getElementById('comparison').textContent='Historia chwilowo niedostępna.'}}
async function loadWeekly(){try{const r=await fetch('/api/weekly');const w=await r.json();const m=w.metrics||{};document.getElementById('weekly').innerHTML=Object.entries(m).map(([k,z])=>box(k,`7d: ${v(z.current?.average)} · poprzednie: ${v(z.previous?.average)} · Δ ${v(z.delta)}`)).join('')}catch(e){document.getElementById('weekly').textContent='Raport tygodniowy chwilowo niedostępny.'}}
