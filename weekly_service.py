...
def generate_weekly_report(week_start: date | None = None) -> dict:
    ...
    start = week_start or (now.date() - timedelta(days=now.weekday()))
    report = {"week_start": start.isoformat(), "generated_at": now.isoformat(), "metrics": {}}
...
def save_weekly_report(report: dict) -> None:
    with connection() as conn:
        conn.execute(
            ...
            (report["week_start"], datetime.now(timezone.utc), Jsonb(report)),
        )
        conn.commit()
