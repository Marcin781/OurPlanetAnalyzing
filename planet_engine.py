...
async def get_text(url: str) -> str:
    async with httpx.AsyncClient(timeout=TIMEOUT, follow_redirects=True, headers={"User-Agent": "Dziennik-Planety/3.0"}) as client:
...
async def noaa_co2() -> dict[str, Any]:
    ...
    for row in csv.reader(io.StringIO(text)):
        if not row or row[0].strip().startswith("#") or row[0].strip().lower() == "year":
            continue
        try:
            # NOAA format: year, month, decimal date, monthly average, deseasonalized, ...
...
async def nasa_global_temperature() -> dict[str, Any]:
    # NASA publishes the same GISTEMP v4 table as TXT and CSV. TXT is used first
    # because it is the stable documented download endpoint; CSV is the fallback.
    urls = [
    ...
    for url in urls:
        try:
            ...
            if text and "Land-Ocean: Global Means" in text:
                break
        except Exception as exc:
            last_error = exc
    if not text or "Land-Ocean: Global Means" not in text:
...
async def legnica_weather() -> dict[str, Any]:
    ...
    # Soil moisture is an hourly variable in Open-Meteo; requesting it as a
    # current variable causes the whole response to fail on some API versions.
    data = await get_json(
    ...
    avg_temp = mean(temp7) if temp7 else None
    avg_hum = mean(hum7) if hum7 else None
    avg_soil = mean(soil7) if soil7 else None
    ...
    return {
        ...
        "history": {
            "rain_7d_mm": round(rain7_total, 1),
            "rain_14d_mm": round(rain14_total, 1),
            "avg_temp_7d_c": round(avg_temp, 1) if avg_temp is not None else None,
            "avg_humidity_7d_pct": round(avg_hum, 1) if avg_hum is not None else None,
            "avg_soil_moisture": round(avg_soil, 3) if avg_soil is not None else None,
        },
        "forecast": {"times": data.get("hourly", {}).get("time", [])[-168:], "rain_mm": data.get("hourly", {}).get("precipitation", [])[-168:]},
        "mushroom_score": round(max(0, min(100, score))),
...
async def source_status() -> dict[str, Any]:
    ...
    for name, fn in jobs:
        try:
            ...
            results[name] = {"source": name, "status": "error", "error_type": type(exc).__name__, "error": str(exc)}
    return results
