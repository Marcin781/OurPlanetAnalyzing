...
async def noaa_co2() -> dict[str, Any]:
    ...
    previous_year = next((r for r in reversed(rows[:-1]) if latest and r["year"] == latest["year"] - 1 and r["month"] == latest["month"]), None)
...
async def legnica_weather() -> dict[str, Any]:
    ...
    base_params = {"latitude": lat, "longitude": lon, "current": "temperature_2m,relative_humidity_2m,precipitation,rain", "hourly": "temperature_2m,relative_humidity_2m,precipitation", "past_days": 30, "forecast_days": 7, "timezone": "Europe/Warsaw"}
    data = await get_json("https://api.open-meteo.com/v1/forecast", base_params)
    soil_data = None
    try:
        soil_params = dict(base_params)
...
async def source_status() -> dict[str, Any]:
    ...
    for name, fn in jobs:
        ...
        except Exception as exc: results[name] = {"source": name, "status": "error", "error_type": type(exc).__name__, "error": str(exc)}
    return results
